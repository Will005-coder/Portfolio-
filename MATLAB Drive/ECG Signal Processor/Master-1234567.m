...
% The struggles we had were finding a efficient way to make the plot window
% clear. We solved this by making helper functions. Another issue we had
% was to properly load the mat files in order to use them properly. This
% was solved by clearing b1 after each load. Initially had trouble with
% bounds for the graphs but tweaking the x and ylim values helped.
...
percentOffset = 0.05; % 10% offset on range
...
% Find the target indices of the 100th time instance
yLimIndxEx = find(yValsEx == 100);
yLimEx = addBorder(yValsEx(1:yLimIndxEx));
ylim(yLimEx);
% x-value window from 101 to 100 for data visual clarity
xlim([101 110]);
...
% Heart Rate peaks
% Use findpeaks to get points where
[peakVals, locationIndex] = findpeaks(yValsEx);

% convert location-index vector to plottable position vector
pTimeVecEx = timeVec(locationIndex);

% formatted peakValue indicator offset from the plot for clarity
formatPeakValEx = clarityOffset(peakVals, yLimEx);
