...
% with overlapping the 3-wave cycles on each other to measure trigger
% integrity. Resolved by centering each cycle around the x-index for the
% R-wave trigger.
...
% Sam: Had issues understanding which data visualization format was
% required. Resolved by comparing to PDF and adjusting code to overlap
% histograms
...
%% Step 6.1: Plot overlayed plot with formatted transparency
% For resting:
...
% Resting heart rate histogram
histogram(heart_rate_rest, 'Normalization', 'probability');
xlabel('Heart Rate (BPM)', 'FontSize', 14);
ylabel('Probability', 'FontSize', 14);
grid on;
hold on;

% Exercise heart rate histogram
histogram(heart_rate_ex, 'Normalization', 'probability');
xlabel('Heart Rate (BPM)', 'FontSize', 14);
ylabel('Probability', 'FontSize', 14);
title('Resting vs Exercise: Heart Rate Distribution', 'FontSize', 16);
grid on;

legend("Resting Heart Rate", "Exercise Heart Rate");
hold off;
...
% Plot boxchart
figure;
b = boxchart(group, combinedData);  % b stores the BoxChart object
b.BoxWidth = 0.5; 
...
% Force x-ticks to line up with boxes
xticks([1 2 3]);
...
histogram(R_speeds, 'Normalization', 'probability', ...
          'FaceColor', 'b', 'FaceAlpha', 0.6, 'DisplayName', 'R Pulse Speed');

histogram(T_speeds, 'Normalization', 'probability', ...
          'FaceColor', 'r', 'FaceAlpha', 0.6, 'DisplayName', 'T Pulse Speed');

hold off;

xlabel('Pulse Speed (mV/s)', 'FontSize', 14);
ylabel('Probability', 'FontSize', 14);  % change to probability
